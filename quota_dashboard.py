...
---------------------------------------------------------
Builds the UK steel quota dashboard + daily movers + historical trends pages.
The quota list (44 order numbers, category, country, quarterly base) is baked
in below. For each order number it pulls the live remaining balance from the
HMRC trade-tariff API and writes public/index.html, movers.html, trends.html
plus a daily snapshot log (history.json).
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>UK Quota Daily Movers</title>
<style>
...
<body><div class="wrap">
<header><div><h1>UK Steel Quota &mdash; Daily Movers</h1>
...
<div class="summary">
  ...
  <div class="stat crit"><div class="n">%%TOTDRAWN%%</div><div class="l">Total drawn (MT)</div></div>
  <div class="stat crit"><div class="n">%%BIGGEST%%</div><div class="l">Biggest single drop (MT)</div></div>
</div>
%%BODY%%
...
Tracking begins from the first snapshot, so day-over-day figures build up over time. Source: HMRC trade-tariff quota API.</footer>
</div></body></html>"""
